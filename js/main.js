...
function startCountdown(remaining) {
  ...
  countdownDiv.innerHTML = "✅ يمكنك الآن الحصول على مهمة جديدة!";
  taskButton.disabled = false;

  // إشعار بأن المهمة جاهزة
  if ("Notification" in window && Notification.permission === "granted") {
    new Notification("📌 مهمتك جاهزة!", {
      body: "تحرك الأن لمهمتك الجديده🌟",
      icon: "imgs/praying.png"
    });
  }
}
 else {
...
document.addEventListener("DOMContentLoaded", () => {
  new Choices("#reciterSelect", {
    searchEnabled: true,
    itemSelectText: "",
  });

  new Choices("#surahSelect", {
    searchEnabled: true,
    itemSelectText: "",
  });
});
